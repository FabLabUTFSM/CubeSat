...
#define capacity 553 //peso del json
#define SEALEVELPRESSURE_HPA 1013.25 //config sensor de presion
uint32_t timer = millis();//Para el GPS
...
Adafruit_GPS GPS(&Wire); //Conectar GPS para funcionar por I2C. 

//Coniguracion GPS
// Set GPSECHO to 'false' to turn off echoing the GPS data to the Serial console
// Set to 'true' if you want to debug and listen to the raw GPS sentences
#define GPSECHO false //Averiguar que significa
...
void setup() {
  ...
  if(!accelmag.begin(ACCEL_RANGE_4G)){
   ...
   if(!gyro.begin()) /* Initialise the sensor */
    ...
    GPS.sendCommand(PMTK_SET_NMEA_UPDATE_1HZ); // 1 Hz update rate | Set the update rate
}
